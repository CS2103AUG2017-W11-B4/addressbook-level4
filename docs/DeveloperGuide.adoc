...
link:https://docs.oracle.com/javase/8/javafx/api/javafx/collections/transformation/FilteredList.html[`FilteredList`]
...
For `PersonContainsFieldsPredicate#test` we iterate through all `Predicates` and return `false` if any of their `#test` call returns false. Thus this `Predicate` will check for all matches before return `true` (ie an *AND* find).
